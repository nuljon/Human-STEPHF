...
class HapticSensor
{
  ...
  private:
    ...
    static const uint8_t supplyPin = 19;
    ...
    CapacitiveSensor capSensor = CapacitiveSensor(supplyPin, signalPin);
    ...
    float readSensor(float *sensorHistory);
    // feedbackSensor() returns the haptic response value. It requires passing a pointer to sensorHistory and the maximum feedback limit.
    int feedbackSensor(float *sensorHistory, int vMax = 250);
    // spinHS() transmits the feedback response. It requires passing the feedback value.
    void spinHS(int v);
